    ...
    if path_residents.lower().endswith(('.xlsx', '.xls')):
        try:
            ...
            if header_row is None:
                header_row = 2
            df_t14 = pd.read_excel(
                path_residents, sheet_name='T14', header=header_row, engine='openpyxl'
            )
        except Exception:
            df_t14 = None
        ...
        if df_t14 is not None:
            ...
            for c, lc in cols_low.items():
                if 'postleitzahl' in lc or lc == 'plz':
                    plz_col = c
                if 'insgesamt' in lc or lc == 'ins-' or 'gesamt' in lc:
                    total_col = c
            ...
            if plz_col is not None and total_col is not None:
                ...
                df_res['einwohner'] = pd.to_numeric(
                    df_res['einwohner'], errors='coerce'
                ).fillna(0).astype(int)

                # Attach PLZ centroid
                df_geodat_plz_loc = df_geodat_plz.copy()
                df_geodat_plz_loc['geometry'] = gpd.GeoSeries.from_wkt(
                    df_geodat_plz_loc['geometry']
                )
                ...
                merged_plz = df_res.merge(
                    gdf_plz[['PLZ', 'centroid']], left_on='plz', right_on='PLZ', how='left'
                )
                merged_plz['lat'] = merged_plz['centroid'].apply(
                    lambda g: g.y if g is not None else None
                )
                merged_plz['lon'] = merged_plz['centroid'].apply(
                    lambda g: g.x if g is not None else None
                )
                df_residents = merged_plz[
                    ['plz', 'einwohner', 'lat', 'lon']
                ].copy().dropna(subset=['plz'])
