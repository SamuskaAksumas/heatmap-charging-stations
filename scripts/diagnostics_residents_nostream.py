...
print('cols lower sample:', list(cols_low.items())[:8])
...
for c, lc in cols_low.items():
    if 'bezirk' in lc:
        district_col = c
    if 'insgesamt' in lc or 'gesamt' in lc:
        total_col = c
...
# choose name col (prefer 'Gemeinde_n' if present)
...
# ensure CRS match
print('plz crs', gdf_plz_centroids.crs, 'bez crs', gdf_bez.crs)
...
print('plz-level einwohner sum (total residents distributed):', int(df_residents_plz['einwohner'].sum()))

# create GeoDataFrame merging with df_geodat_plz
sorted_df2 = df_residents_plz.merge(df_geodat_plz, left_on='plz', right_on='PLZ', how='left')
print('merged with geodata rows', len(sorted_df2))
sorted_df2 = sorted_df2.dropna(subset=['geometry'])
sorted_df2['geometry'] = gpd.GeoSeries.from_wkt(sorted_df2['geometry'])
print('final plz geodata rows', len(sorted_df2))

print('final Einwohner stats', sorted_df2['einwohner'].describe())
